...
from src.prepare_datasets import prepare_data, add_interactions
...
datasets = {1462: 'Class', 871: 'binaryClass', 885: 'binaryClass', 1120: 'class:', 994: 'binaryClass', 1021: 'binaryClass',  847: 'binaryClass' }

classifiers = {
    'Logistic Regression (SGD) with interactions': LogisticRegression(add_interactions=True, learning_rate=0.01, max_iter=1000, tolerance=1e-10),
    'Logistic Regression (SGD)': LogisticRegression(add_interactions=False, learning_rate=0.01, max_iter=100, tolerance=1e-10),
...
def compare_with_different_classifiers():
    ...
    for i, (dataset_number, target_column) in enumerate(datasets.items(), start=1):
        ...
        for name, model in list(classifiers.items())[1:]:
            ...
            results.append({'Dataset': f'Dataset_{i}', 'Classifier': name, 'Avg_Balanced_Accuracy': avg_accuracy})
...
def compare_w_wo_interactions():
    ...
    for i, (dataset_number, target_column) in enumerate(list(datasets.items())[:3], start=1):
        X, y = prepare_data(dataset_number, target_column)
        print(f'Dataset {i}:')
        ...
        for name, model in list(classifiers.items())[:2]:
            ...
            results.append({'Dataset': f'Dataset_{i}', 'Classifier': name, 'Balanced_Accuracy': accuracy})
